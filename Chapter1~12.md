...
…
규율의 확립은 더 많은 자유로 연결될 때가 많지만, 어떤 팀에는 오히려 독으로 작용하기도 한다. 지나친 규율을 강요한 나머지, 팀이나 리더가 자유롭게 생각하고 결정할 자유를 제약하는 것이다. 임무를 직접 수행하는 일선 지휘관이나 부대가 상황에 맞게 적응하는 능력이 부족하면 이는 팀 전체의 성과를 저해하는 결과를 초래한다. 그러므로 규율과 자유 사이에서 균형점을 찾아 유지하는 것이 중요하다.*
> 
- 규율은 최대한의 효율을 뽑아내거나, 리스크에서 벗어나기 위한 장치의 역할로써 많이 사용됩니다. 실제 업무에서도 그렇죠. 그리고 이런 장치들에 익숙해지면 이를 스스로 통제할 수 있게 되어 익숙해진 불편함 속에서 최선의 결과를 뽑아낼 수 있게 됩니다.
- 하지만 인용된 글에서도 말하듯, 언제나 이것이 적용되진 않습니다. 세상에 항상 올바른 규칙이란 존재하지 않으니까요. 규율은 상황에 맞게 조금 느슨하게 혹은 조금 더 엄격하게 재정의되어야 합니다. 그리고 이는 일의 중요도 혹은 팀의 적응도나 상황에 따라서 유연하게 조절되어야 합니다. 이를 잘 판단하는 것 역시 리더의 재량이겠죠.

---

### 정리

- 많은 이야기들이 나왔지만 결국 중요한 것은 ‘어떤 마음가짐으로 일을 대해야 하는가’ 였습니다. 이는 **극한의 오너십** 이라는 한 단어로 표현할 수 있었죠.
- 리더가 갖춰야할 자질은 무엇이 있는지도 많이 언급됐습니다. 반드시 경력이 쌓이고, 나이를 먹어야만, 직급이 높아져야만 리더가 되는 건 아닐겁니다. 리더가 되는 순간, 되어야만 하는 순간 역시 언제든 다가올 수 있죠. 그 때에 맞춰 능력을 키우는 것은 너무 늦습니다. 이런 서적들을 통해 좋은 리더가 되는 법을 인지하고 자연스럽게 몸에서 배어나오게 훈련하고 있어야 겠네요.